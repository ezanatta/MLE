...
c     coordinate (xs,ys,x,y,ysi)       open(44,file="coord.dat",status="unknown")
...
c                        sigma_phi(k)=sqrt((sigma_r(k)**2)*0.5)
     ...
     &                         sigma_h(g),j,k,l,g
                           ...
                           do ggg=1,nvh,step
                              v_h(ggg)=(vvh+ggg)
                             write(*,*)'3sigma_hmod=',vp,vr,vphi,vh,vvh,
     &                         v_h(ggg),j,k,l,g,ggg
...
c                     if((p0+p1+p2+p3-1).eq.0)chi2=6.63/2
c                     if((p0+p1+p2+p3-1).eq.1)chi2=5.39/2
c                     if((p0+p1+p2+p3-1).eq.2)chi2=4.11/2
c                     if((p0+p1+p2+p3-1).eq.3)chi2=2.77/2

                     chi2=2.77/2

              write(*,*)'yuo have',5-(p0+p1+p2+p3-1),'free parameters'
