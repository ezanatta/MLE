...
c     f
c       open(18,file="N2768_f.GC.dat",status="old")
c     input created with create_input.f
       open(21,file="input_params2.dat",status="old")
...
c         In case your PNe coordinates are in RA and Dec
          ...
          read(11,*,end=1111)id(i),ra(i),dec(i),bc(i),vhel(i),s,col(i)

          

          write(*,*)id(i),xpix(i),ypix(i),ra(i),
     &                       ram(i),ras(i),bc(i), redc(i),
     &                       dec(i),decm(i),decs(i),s,s,vhel(i)  
...
c          vhel_av(i)=vhel(i)-av

c          vhel_av(i)=vhel(i)-v_edo

          vhel_av(i)=vhel(i)-vned
...
c                        sigma_phi(k)=sqrt((sigma_r(k)**2)*0.5)
     ...
     &                         sigma_h(g),j,k,l,g
                           ...
                           do ggg=1,nvh,step
                              v_h(ggg)=(vvh+ggg)
                             write(*,*)'3vhalo_hmod=',vp,vr,vphi,vh,vvh,
     &                         'v',v_h(ggg),j,k,l,g,'ggg',ggg
...
c     in case of free photometric distribution
                       ...
                       write(*,*)'cont',cont,'Lmax',Lmax,'max',max,
     &                  'chi2',chi2,'lnL',lnL,'da qui'

                           if((cont.eq.2).and.(lnL.gt.(Lmax-chi2)))
     &                      then
                              write(*,*)'cont2',Lmax,max,chi2,lnL
...
c                  if(cont.eq.3)then
                   ...
                   write(*,*)'err', errj_min+vp,errj_max+vp,errk_min+vr, 
     &                         errk_max+vr,errl_min+vphi,errl_max+vphi, 
     &                         errg_min+vh,errg_max+vh,errf_min,
     &                         errf_max,errggg_min+vvh,errggg_max+vvh
...
c                  endif
               ...
               close(10)
c
 20          format(12(1x,f8.3))
 1010        format(6(1x,f8.3),1(1x,i1),4(1x,f8.3))
c rotation curve
...
c                      write(*,*)velavB(j),denB(j),j,sigmaB(j),supp(j)
                       ...
                       write(*,*)velav(j),den(j),j,sigmav(j),supp(j)

c                   if(j.eq.nmax)supp(j)=450
                   
                      write(32,*)velavB(j),sigmaB(j),binav(j),
     &                  velav(j),sigmav(j),j,den(j)
...
c     if(j.eq.nmax)supp(j)=450
                ...
                write(16,*)velavB(j),sigmaB(j),binav(j),
     &               velav(j),sigmav(j),j,den(j)
             enddo
