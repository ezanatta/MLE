...
c     &                               (2*(sigma_h(g)**2))
                                 ...
                                 like=like+log((fb(i)*exp(-(v_halo(i)))/
     &                                sigma_h(g)))
...
c                           write(*,*)'f=',phot(h),
c     &                     ' lnL=', lnL,j,k,l,g,h,mm(h),chi2,oo,'bin',h
...
c     in case of free photometric distribution
     ...
     &                      then
                              ...
                              write(*,*)'err_g',g,errg_min,errg_max
                              if(g.lt.errg_min) errg_min=g
                              if(g.gt.errg_max) errg_max=g

                           write(*,*)'err_ggg',ggg,errggg_min,errggg_max
...
c                     if((p0+p1+p2+p3).eq.0)chi2=6.63/2
c                     if((p0+p1+p2+p3).eq.1)chi2=5.39/2
c                     if((p0+p1+p2+p3).eq.2)chi2=4.11/2
c                     if((p0+p1+p2+p3).eq.3)chi2=2.77/2
