...
def plot_vmap(vn):
    ...
    filename = "MS_NGC5866_r7_C2D.fits"
    ...
    filename = "PXF_bin_MS_NGC5866_r7_idl.fits.gz"
    hdu = pyfits.open(filename)
    table = hdu[1].data
    ...
    gal_center_x = hdu.header['CRPIX1']
    gal_center_y = hdu.header['CRPIX2']
    
    fig, ax = plt.subplots()
#    plt.imshow(hdu.data, origin='lower',cmap='gray')
    plt.scatter(DRA, DDEC, c=vcmap, vmin=Vmin, vmax=Vmax)
#    plt.plot(gal_center_x, gal_center_y, 'ro')
    plt.xlim(min(DRA)-300, max(DRA)+100)
    plt.ylim(min(DDEC)-300, max(DDEC)+100)
...
with open(galinput, 'r') as f:
    inp = [x1.split(' ')[0] for x1 in open(galinput).readlines()]    
